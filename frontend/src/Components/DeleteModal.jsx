...
const DeleteModal = ({deleteModal,setDeleteModal,item,refresh,setRefresh}) => {
    const [open,setOpen] = useState(true)
    const cancelButtonRef=useRef(null)
    const handleDelete=async()=>{
        try{
            
            const response= await axiosInstance.delete('trade/',{params:{id:item.id}})
            if(response.data.status===200){

                setDeleteModal(false)
                
                setRefresh(!refresh)
                toast.success('Seccesfully Deleteed!!')
            }
                                        ...
                                        <div className="relative w-full max-h-full">
                                            <div className="relative bg-white rounded-lg shadow dark:bg-gray-700">
                                                ...
                                                <div className="p-6 text-center">
                                                    ...
                                                    {  <h3 className="mb-5 text-lg font-normal text-gray-500 dark:text-gray-400">Are you sure want to Delete ?</h3>}
                                                   
                                                    <button  onClick={handleDelete} data-modal-hide="popup-modal" type="button" className="text-white bg-myRed focus:ring-4 focus:outline-none focus:ring-red-300 dark:focus:ring-red-800 font-medium rounded-lg text-sm inline-flex items-center px-5 py-2.5 text-center mr-2">
                                                        Yes, I'm sure
                                                    </button>
                                                    <button  onClick={()=>{
                                                        setDeleteModal(false)
